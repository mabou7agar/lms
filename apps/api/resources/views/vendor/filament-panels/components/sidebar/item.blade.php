{{--
    OVERRIDE of vendor/filament/filament/resources/views/components/sidebar/item.blade.php
    (package filament/filament, version v4.11.8).

    Sole change vs upstream: an `aria-label` on the item's <a>. When the sidebar is COLLAPSED the
    visible label <span> is `x-show`-hidden, leaving the link with only an icon + a visual tooltip
    and NO accessible name — a serious axe `link-name` violation. The aria-label (the item's own
    label text) gives every item a stable accessible name in both states; it equals the visible
    text when expanded, so WCAG 2.5.3 (label-in-name) is satisfied.

    Keep this file byte-for-byte in sync with upstream on every Filament upgrade — diff against the
    vendor path above and re-apply only the single aria-label line.
--}}
@props([
    'active' => false,
...
@php
    ...
    $accessibleName = trim(strip_tags($slot->toHtml()));
@endphp
