...
class ReportingService extends BaseService
{
    ...
    public function revenue(CarbonImmutable $from, CarbonImmutable $to): array
    {
        ...
        $byCourse = OrderItem::query()
            ...
            ->toBase()
            ->selectRaw('courses.title as course, SUM(order_items.unit_amount_minor * COALESCE(order_items.quantity, 1)) as revenue_minor, COUNT(*) as units')
            ->groupBy('courses.id', 'courses.title')
            ->orderByRaw('SUM(order_items.unit_amount_minor * COALESCE(order_items.quantity, 1)) DESC')
            ->limit(25)
            ->get()
    ...
    public function commerce(CarbonImmutable $from, CarbonImmutable $to): array
    {
        ...
        $topProducts = OrderItem::query()
            ...
            ->whereBetween('orders.paid_at', [$from, $to])
            ->toBase()
            ->selectRaw('order_items.title as product, SUM(order_items.unit_amount_minor * COALESCE(order_items.quantity, 1)) as revenue_minor, COUNT(*) as units')
    ...
    public function coursePerformance(CarbonImmutable $from, CarbonImmutable $to, int $page, int $perPage): array
    {
        ...
        $revenue = OrderItem::query()
            ...
            ->toBase()
            ->selectRaw('product_courses.course_id as course_id, SUM(order_items.unit_amount_minor * COALESCE(order_items.quantity, 1)) as revenue_minor')
    ...
    public function instructorPerformance(CarbonImmutable $from, CarbonImmutable $to, int $page, int $perPage): array
    {
        ...
        $revenue = OrderItem::query()
            ...
            ->whereBetween('orders.paid_at', [$from, $to])
            ->toBase()
            ->selectRaw('course_trainer.user_id as instructor_id, SUM(order_items.unit_amount_minor * COALESCE(order_items.quantity, 1)) as revenue_minor')
            ->groupBy('course_trainer.user_id')
    ...
    public function adminSummary(CarbonImmutable $from, CarbonImmutable $to): array
    {
        ...
        $byProductType = DB::table('order_items')
            ...
            ->whereNotNull('orders.paid_at')
            ->whereBetween('orders.paid_at', [$from, $to])
            ->selectRaw('products.type as product_type, COUNT(*) as units, COALESCE(SUM(order_items.unit_amount_minor * COALESCE(order_items.quantity, 1)), 0) as revenue_minor')
        ...
        $topBundles = DB::table('order_items')
            ...
            ->whereNotNull('orders.paid_at')
            ->whereBetween('orders.paid_at', [$from, $to])
            ->selectRaw('products.title as bundle, COUNT(*) as units, COALESCE(SUM(order_items.unit_amount_minor * COALESCE(order_items.quantity, 1)), 0) as revenue_minor')
    ...
    private function topCoursesByRevenue(CarbonImmutable $from, CarbonImmutable $to): array
    {
        return DB::table('order_items')
            ...
            ->whereNotNull('orders.paid_at')
            ->whereBetween('orders.paid_at', [$from, $to])
            ->selectRaw('courses.title as course, COUNT(*) as units, COALESCE(SUM(order_items.unit_amount_minor * COALESCE(order_items.quantity, 1)), 0) as revenue_minor')
            ->groupBy('courses.id', 'courses.title')
            ->orderByRaw('SUM(order_items.unit_amount_minor * COALESCE(order_items.quantity, 1)) DESC')
            ->limit(10)
            ->get()
